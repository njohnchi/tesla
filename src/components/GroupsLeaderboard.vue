...
<template>
  <Card>
    ...
    <div class="container">
      <div class="banner-wrapper" v-for="group in groups" :key="group.name">
        <div class="content">
          <h3 class="topic-name">{{ group.name }}</h3>
          <div class="status">
            <span class="correct-text">{{ group.points }} Points / User - {{ group.correct }}% correct</span>
            <span>
              {{ Math.abs(group.rating) }}
              <svg v-if="group.rating >= 0" width="12" height="11" viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg">
